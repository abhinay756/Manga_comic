...
class TrackedClients:
    ...
    def anthropic_hello(self, model: str | None = None) -> str:
        default_model = (
            env("OMNIROUTE_ANTHROPIC_MODEL") or "auto/cheap"
            if USE_OMNIROUTE
            else env("ANTHROPIC_HELLO_MODEL", "claude-haiku-4-5-20251001")
        )
    ...
    def openai_hello(self, model: str | None = None) -> str:
        default_model = (
            env("OMNIROUTE_OPENAI_MODEL") or "auto/cheap"
            if USE_OMNIROUTE
            else env("OPENAI_HELLO_MODEL", "gpt-4.1-mini")
        )
        model = model or default_model
